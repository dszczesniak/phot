...
from ..models import Gallery, Family
...
@register.inclusion_tag('photologue/tags/next_in_family.html')
def next_in_family(photo, family):
    return {'photo': photo.get_next_in_family(family)}
...
@register.inclusion_tag('photologue/tags/prev_in_family.html')
def previous_in_family(photo, family):
    return {'photo': photo.get_previous_in_family(family)}
...
@register.simple_tag
def cycle_lite_family(family_title, height, width):
    """Generate image tags for jquery slideshow gallery.
    See http://malsup.com/jquery/cycle/lite/"""
    html = ""
    first = 'class="first"'
    for p in Family.objects.get(title=family_title).public():
        html += u'<img src="%s" alt="%s" height="%s" width="%s" %s />' % (
            p.get_display_url(), p.title, height, width, first)
        first = None
    return html
...
class PhotoFamilyNode(template.Node):

    def __init__(self, family, photosize, css_class):
        self.family = family
        self.photosize = photosize
        self.css_class = css_class
    ...
    def render(self, context):
        try:
            a = template.resolve_variable(self.family, context)
        except:
            a = self.family
