...
from .models import Gallery, Family, Photo, PhotoEffect, PhotoSize, \
    Watermark
...
class FamilyAdmin(admin.ModelAdmin):
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        ...
        if db_field.name == "sites":
            kwargs["initial"] = [Site.objects.get_current()]
        return super(FamilyAdmin, self).formfield_for_manytomany(db_field, request, **kwargs)
    ...
    def save_related(self, request, form, *args, **kwargs):
        ...
        different Sites - it might cause much confusion. So let them know.
        """
        super(FamilyAdmin, self).save_related(request, form, *args, **kwargs)
        orphaned_photos = form.instance.orphaned_photos()
        if orphaned_photos:
            msg = ungettext(
                'The following photo does not belong to the same site(s)'
    ...
    def add_to_current_site(modeladmin, request, queryset):
        current_site = Site.objects.get_current()
        current_site.family_set.add(*queryset)
        msg = ungettext(
            "The gallery has been successfully added to %(site)s",
            "The galleries have been successfully added to %(site)s",
            len(queryset)
    ...
    def remove_from_current_site(modeladmin, request, queryset):
        current_site = Site.objects.get_current()
        current_site.family_set.remove(*queryset)
        msg = ungettext(
            "The gallery has been successfully removed from %(site)s",
            "The selected galleries have been successfully removed from %(site)s",
            len(queryset)
    ...
    def add_photos_to_current_site(modeladmin, request, queryset):
        photos = Photo.objects.filter(families__in=queryset)
        current_site = Site.objects.get_current()
        current_site.photo_set.add(*photos)
        msg = ungettext(
            'All photos in gallery %(families)s have been successfully added to %(site)s',
            'All photos in galleries %(families)s have been successfully added to %(site)s',
            len(queryset)
        ) % {
            'site': current_site.name,
            'galleries': ", ".join(["'{0}'".format(family.title)
    ...
    def remove_photos_from_current_site(modeladmin, request, queryset):
        ...
        current_site.photo_set.remove(*photos)
        msg = ungettext(
            'All photos in gallery %(families)s have been successfully removed from %(site)s',
            'All photos in galleries %(families)s have been successfully removed from %(site)s',
            len(queryset)
        ) % {
            'site': current_site.name,
            'galleries': ", ".join(["'{0}'".format(family.title)
